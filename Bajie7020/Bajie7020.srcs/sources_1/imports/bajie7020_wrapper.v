//Copyright 1986-2022 Xilinx, Inc. All Rights Reserved.
//--------------------------------------------------------------------------------
//Tool Version: Vivado v.2022.1 (lin64) Build 3526262 Mon Apr 18 15:47:01 MDT 2022
//Date        : Sat Jun 24 20:20:28 2023
//Host        : Penta0308-15U560-U running 64-bit Ubuntu 22.04.2 LTS
//Command     : generate_target bajie7020_wrapper.bd
//Design      : bajie7020_wrapper
//Purpose     : IP block netlist
//--------------------------------------------------------------------------------
`timescale 1 ps / 1 ps

module bajie7020_wrapper
   (CAN0_rx,
    CAN0_tx,
    DDR_addr,
    DDR_ba,
    DDR_cas_n,
    DDR_ck_n,
    DDR_ck_p,
    DDR_cke,
    DDR_cs_n,
    DDR_dm,
    DDR_dq,
    DDR_dqs_n,
    DDR_dqs_p,
    DDR_odt,
    DDR_ras_n,
    DDR_reset_n,
    DDR_we_n,
    FIXED_IO_ddr_vrn,
    FIXED_IO_ddr_vrp,
    FIXED_IO_mio,
    FIXED_IO_ps_clk,
    FIXED_IO_ps_porb,
    FIXED_IO_ps_srstb,
    RFB_DATAVALID,
    RFB_DATA_data_n,
    RFB_DATA_data_p,
    RFB_DCLK_clk_n,
    RFB_DCLK_clk_p,
    RFB_FCLK_clk_n,
    RFB_FCLK_clk_p);
  input CAN0_rx;
  output CAN0_tx;
  inout [14:0]DDR_addr;
  inout [2:0]DDR_ba;
  inout DDR_cas_n;
  inout DDR_ck_n;
  inout DDR_ck_p;
  inout DDR_cke;
  inout DDR_cs_n;
  inout [3:0]DDR_dm;
  inout [31:0]DDR_dq;
  inout [3:0]DDR_dqs_n;
  inout [3:0]DDR_dqs_p;
  inout DDR_odt;
  inout DDR_ras_n;
  inout DDR_reset_n;
  inout DDR_we_n;
  inout FIXED_IO_ddr_vrn;
  inout FIXED_IO_ddr_vrp;
  inout [53:0]FIXED_IO_mio;
  inout FIXED_IO_ps_clk;
  inout FIXED_IO_ps_porb;
  inout FIXED_IO_ps_srstb;
  input RFB_DATAVALID;
  input [3:0]RFB_DATA_data_n;
  input [3:0]RFB_DATA_data_p;
  input [0:0]RFB_DCLK_clk_n;
  input [0:0]RFB_DCLK_clk_p;
  input [0:0]RFB_FCLK_clk_n;
  input [0:0]RFB_FCLK_clk_p;

  wire CAN0_rx;
  wire CAN0_tx;
  wire [14:0]DDR_addr;
  wire [2:0]DDR_ba;
  wire DDR_cas_n;
  wire DDR_ck_n;
  wire DDR_ck_p;
  wire DDR_cke;
  wire DDR_cs_n;
  wire [3:0]DDR_dm;
  wire [31:0]DDR_dq;
  wire [3:0]DDR_dqs_n;
  wire [3:0]DDR_dqs_p;
  wire DDR_odt;
  wire DDR_ras_n;
  wire DDR_reset_n;
  wire DDR_we_n;
  wire FIXED_IO_ddr_vrn;
  wire FIXED_IO_ddr_vrp;
  wire [53:0]FIXED_IO_mio;
  wire FIXED_IO_ps_clk;
  wire FIXED_IO_ps_porb;
  wire FIXED_IO_ps_srstb;
  wire RFB_DATAVALID;
  wire [3:0]RFB_DATA_data_n;
  wire [3:0]RFB_DATA_data_p;
  wire [0:0]RFB_DCLK_clk_n;
  wire [0:0]RFB_DCLK_clk_p;
  wire [0:0]RFB_FCLK_clk_n;
  wire [0:0]RFB_FCLK_clk_p;

  bajie7020 bajie7020_i
       (.CAN0_rx(CAN0_rx),
        .CAN0_tx(CAN0_tx),
        .DDR_addr(DDR_addr),
        .DDR_ba(DDR_ba),
        .DDR_cas_n(DDR_cas_n),
        .DDR_ck_n(DDR_ck_n),
        .DDR_ck_p(DDR_ck_p),
        .DDR_cke(DDR_cke),
        .DDR_cs_n(DDR_cs_n),
        .DDR_dm(DDR_dm),
        .DDR_dq(DDR_dq),
        .DDR_dqs_n(DDR_dqs_n),
        .DDR_dqs_p(DDR_dqs_p),
        .DDR_odt(DDR_odt),
        .DDR_ras_n(DDR_ras_n),
        .DDR_reset_n(DDR_reset_n),
        .DDR_we_n(DDR_we_n),
        .FIXED_IO_ddr_vrn(FIXED_IO_ddr_vrn),
        .FIXED_IO_ddr_vrp(FIXED_IO_ddr_vrp),
        .FIXED_IO_mio(FIXED_IO_mio),
        .FIXED_IO_ps_clk(FIXED_IO_ps_clk),
        .FIXED_IO_ps_porb(FIXED_IO_ps_porb),
        .FIXED_IO_ps_srstb(FIXED_IO_ps_srstb),
        .RFB_DATAVALID(RFB_DATAVALID),
        .RFB_DATA_data_n(RFB_DATA_data_n),
        .RFB_DATA_data_p(RFB_DATA_data_p),
        .RFB_DCLK_clk_n(RFB_DCLK_clk_n),
        .RFB_DCLK_clk_p(RFB_DCLK_clk_p),
        .RFB_FCLK_clk_n(RFB_FCLK_clk_n),
        .RFB_FCLK_clk_p(RFB_FCLK_clk_p));
endmodule
